...
function setDisplayMode(mode) {
  displayMode = mode;
  // keep radio group and mobile select in sync
    ...
    try {
      ...
      const params = {
        ...
        amplitude: meta.amplitude != null
          ? meta.amplitude
          : (ch.peakRange ? (ch.peakRange[0] + ch.peakRange[1]) / 2 : 0.7),
        delay:    meta.delay    || 0,
...
function initUI() {
  ...
  /* Responsive: show/hide mobile elements */
  function applyResponsive() {
    const mobile = isMobile();
    showMobileElements(mobile);
    // ensure panel is not stuck open after resize to desktop
